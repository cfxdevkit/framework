...
export async function checkPackageExports() {
  ...
  for (const packageJson of await findFiles(root, 'package.json')) {
    ...
    for (const [exportPath, target] of Object.entries(pkg.exports)) {
      const exportTarget = target as { import?: unknown };
      const importPath = typeof target === 'object' && target ? exportTarget.import : undefined;
...
export function suggestValidationCommands(changed) {
  ...
  if (!changed.length) return ['pnpm run check:docs', 'pnpm run check:eval'];
  ...
  if (
    ...
    commands.add('pnpm run check:docs');
  }
  if (touchesDocsSite) {
  ...
  if (touchesWorkflow || touchesDocsDeploy || touchesAnsible) {
    commands.add('pnpm run check:ci');
