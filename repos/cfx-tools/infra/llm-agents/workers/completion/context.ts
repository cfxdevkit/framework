...
const baseContextFiles = [
  ...
  'artifacts/llm/corpus/manifest.json',
] as const;
...
export async function buildBaseContext(opts: { quick?: boolean } = {}) {
  const files = await Promise.all(baseContextFiles.map((file) => readContextFile(file)));
  return files
    .filter(Boolean)
    .join('\n\n')
...
export async function readContextFile(path: string): Promise<string> {
  try {
    const filePath = path.startsWith('/') ? path : join(root, path);
...
async function changedFilesBlock(): Promise<string> {
...
async function gitDiffBlock(): Promise<string> {
...
export async function commitPreflightBlock(): Promise<string> {
...
export async function writeLlmReport(action: string, response): Promise<void> {
  const reportPath = join(artifactsRoot, 'reports', `llm-${action}.md`);
