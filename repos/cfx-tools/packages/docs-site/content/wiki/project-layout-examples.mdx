...
- **Key Features**:
  ...
  - WS protocol ↔ `domains/hardware-bridge`
...
- **Apps**:
  - `showcase-gateway/` — Single entry point (`http://127.0.0.1:5173`)
  - `showcase/` — SDK fundamentals (HD derivation, Core/eSpace, devnode)
  - `showcase-stack/` — Full-stack (backend + wallet)
  - `showcase-browser/` — External wallet flows (MetaMask, Fluent Core)
  - `showcase-backend/` — Backend service (SIWE, devnode, compiler)
  - `hardware-wallet-showcase/` — Keystore management (memory, file, Ledger)
...
- Gateway navigation & panel state (`ShowcaseNav`, `PanelSidebar`, `useActivePanelState`)
...
### Gateway-Driven Showcase

```bash
pnpm showcase
# Starts backend + gateway + all apps
# → http://127.0.0.1:5173
```

The gateway (`showcase-gateway/`) proxies:

| Path | Target |
|------|--------|
| `/showcase/` | `@cfxdevkit/example-showcase` |
| `/stack/` | `@cfxdevkit/example-showcase-stack` |
| `/browser/` | `@cfxdevkit/example-showcase-browser` |
| `/keystores/` | `@cfxdevkit/example-hardware-wallet-showcase` |
| `/api/*` | `@cfxdevkit/example-showcase-backend` |
| `/devnode/*`, `/rpc/*`, `/compile/*` | Same-origin backend routes |
...
Use `VITE_BACKEND_URL` to override backend location.
...
| `domains/automation`, `game-engine`, `hardware-bridge` | Gap | Add domain-specific examples after core flows stabilize |
