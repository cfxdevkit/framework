...
#### `@cfxdevkit/tsconfig` (`tools/tsconfig/`)
...
#### `@cfxdevkit/biome-config` (`tools/biome-config/`)
...
```json
{
  ...
  "extends": ["./tools/biome-config/biome.json"]
}
```

#### `@cfxdevkit/moon-config` (`tools/moon-config/`)
...
1. Edit `tools/tsconfig/base.json`, `tools/biome-config/biome.json`, etc.
...
- **New language?** Add `tools/tsconfig/app-<lang>.json`
- **New security rule?** Extend `scripts/check-secret-leaks.mjs`
- **New task?** Add to `tools/moon-config/templates/`
