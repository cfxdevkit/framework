...
describe('deployContract', () => {
  ...
  it('encodes deploy data, signs, and broadcasts (Core)', async () => {
    ...
    // 25% safety buffer over `cfx_estimateGasAndCollateral` (gasLimit 0x186a0 = 100_000,
    // storageCollateralized 0x40 = 64).
    expect(signed?.gas).toBe(125000n);
    expect(signed?.storageLimit).toBe(80n);
