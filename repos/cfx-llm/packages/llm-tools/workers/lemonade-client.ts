...
import { configPath, defaultBaseUrls, modelPaths } from './lemonade-shared.ts';
...
export async function discoverModels(baseUrls, opts = {}) {
  const attempts = [];
...
export function chooseModel(models, preferredId) {
  if (preferredId) {
