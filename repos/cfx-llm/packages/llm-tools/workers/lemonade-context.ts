...
import { commandBlock, git } from './lemonade-runner.ts';
import { artifactsRoot, root, workerDir } from './lemonade-shared.ts';

export async function buildActionContext(spec, opts = {}) {
  const files = [];
...
export async function buildBaseContext(opts = {}) {
...
export async function commitPreflightBlock() {
  ...
  const blocks = await Promise.all([
    ...
    commandBlock(
      'code hotspot scan',
      'pnpm',
      ['exec', 'tsx', join(workerDir, 'code-hotspots.ts')],
      { maxChars: 12000 },
    ),
