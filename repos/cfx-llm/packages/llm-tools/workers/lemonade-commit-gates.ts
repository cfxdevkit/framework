...
import { execFileAsync, QUALITY_GATES, root, workerDir } from './lemonade-shared.ts';
import { logInfo } from './lemonade-util.ts';
...
export async function runCodeHotspotGate() {
  ...
  try {
    const { stdout, stderr } = await execFileAsync(
      'pnpm',
      ['exec', 'tsx', join(workerDir, 'code-hotspots.ts'), '--fail-on-hard'],
      {
        cwd: root,
