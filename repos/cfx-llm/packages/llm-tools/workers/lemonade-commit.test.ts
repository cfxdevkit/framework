...
import { parseCommitFlags } from './lemonade-commit.ts';
import { fallbackCommitMessage } from './lemonade-commit-message.ts';
...
describe('commit worker wiring', () => {
  it('imports every helper it calls during approval', async () => {
    ...
    expect(source).toMatch(
      /import\s*\{[\s\S]*confirmPrompt[\s\S]*\}\s*from '\.\/lemonade-commit-message\.ts'/,
    );
  });
});
