...
> **Source:** `/home/sp/Documents/Code/devkit/packages/cdk` (`@cfxdevkit/cdk@1.2.5` reference implementation).
...
| Source | Verdict | Target |
|---|---|---|
| `TransactionBatcher` (class) | **RELOCATE** + **RESHAPE** | Signer-aware multisend → `@cfxdevkit/wallet/batched` (functional). Signer-free coalescing → `@cfxdevkit/cdk/batch` as `createBatcher`. |
...
import { formatUnits } from '@cfxdevkit/cdk/units';
...
- [ ] `TransactionBatcher` → `@cfxdevkit/wallet/batched` (signer-aware) + `@cfxdevkit/cdk/batch` (signer-free)
