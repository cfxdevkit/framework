...
The system SHALL display all ERC-20 token allowances the connected wallet has granted to the AutomationManager contract, grouped by token, showing the current on-chain allowance and the total amount committed by active jobs. The `ApprovalWidget` SHALL be rendered as a controlled modal overlay accepting `open: boolean` and `onClose: () => void` props; it SHALL NOT be embedded inline in the page layout.

#### Scenario: Allowances load when modal opens
- **WHEN** an authenticated user opens the Approvals modal (by clicking the "Approvals" button on the home page)
