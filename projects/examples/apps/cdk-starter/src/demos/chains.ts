...
export async function demoChains(): Promise<void> {
  ...
  for (const chain of listChains()) {
    ...
    console.log(
      `    ${network} ${family} ${chain.name.padEnd(18)} id=${chain.id}  "${chain.displayName}"`,
    );
  }
  ...
  console.log(
    `    token:          ${espaceMainnet.nativeToken.symbol} (${espaceMainnet.nativeToken.decimals} dec)`,
  );
  ...
  console.log(
    `    token:          ${coreSpaceMainnet.nativeToken.symbol} (${coreSpaceMainnet.nativeToken.decimals} dec)`,
  );
  ...
  console.log('\n  Filter by family:');
  console.log(
    `    eSpace chains:  ${listChains({ family: 'espace' })
      .map((c) => c.name)
      .join(', ')}`,
  );
  console.log(
    `    Core chains:    ${listChains({ family: 'core' })
      .map((c) => c.name)
      .join(', ')}`,
  );
