/**
 * UnifiedSignPanel — message signing for both spaces side-by-side.
 *
 * Left column  → eSpace: personal_sign (EIP-191) + typed data (EIP-712)
 * Right column → Core: personal_sign + typed data (CIP-23)
 *
...
// ── EIP-712 typed-data template (eSpace) ─────────────────────────────
      ...
      >
        ...
        <div className="sub-section">
          ...
          {pmResult && (
            <div className="result">
              sig : {pmResult.sig}
              {'\n'}verified : {pmResult.verified ? '✓ matches account' : '✗ MISMATCH'}
              <CopyButton text={pmResult.sig} />
            </div>
          )}
        </div>
        ...
        <div className="sub-section" style={{ marginTop: 14 }}>
          ...
          {tdResult && (
            <div className="result">
              sig : {tdResult.sig}
              {'\n'}verified : {tdResult.verified ? '✓ matches account' : '✗ MISMATCH'}
              <CopyButton text={tdResult.sig} />
            </div>
          )}
        </div>
      </ConnectWall>
...
function CoreSection() {
  ...
  // Populate template once account is known
