 ...
 *
 *   1. **eSpace / EVM** — the non-Fluent injected provider selected by
 *      WagmiProviders. Fluent is excluded from wagmi so Core network
 *      switches cannot trigger eSpace switch prompts.
 *
...
import { getFluentProvider, rpcRequestAccounts, useCoreWallet } from '../lib/use-core-wallet.js';
...
export function WalletPickerModal({ open, onClose, section = 'espace' }: WalletPickerModalProps) {
  ...
  const { isConnected: coreIsConnected, isConnecting: coreIsConnecting } = useCoreWallet();
  ...
  const connectCore = async () => {
    ...
    try {
      ...
      if (!p) throw new Error('Fluent is not installed');
      await rpcRequestAccounts(p);
      onClose();
    } catch (e) {
  ...
  // Filter out duplicates in case wagmi surfaces equivalent connector entries.
  ...
  return (
    ...
    >
      {/* Click-outside backdrop. Rendered as a real button under the modal so
          it satisfies a11y rules for interactive static elements. */}
      <button
        type="button"
      ...
      <div className="modal" role="document" style={{ position: 'relative', zIndex: 1 }}>
        ...
        <p className="panel-desc" style={{ marginTop: 4 }}>
          {section === 'espace' ? (
            <>
              EVM / eSpace wallets via <code className="mono">wagmi</code>. Fluent is excluded from
              this path.
            </>
          ) : section === 'core' ? (
            <>
              Conflux Core wallets via the internal <code className="mono">useCoreWallet</code> hook
              (<code className="mono">window.conflux</code>).
            </>
          ) : (
            <>
              Detected providers in this browser. eSpace / EVM wallets are wired through{' '}
              <code className="mono">wagmi</code>. Conflux Core is wired through the internal{' '}
              <code className="mono">useCoreWallet</code> hook.
            </>
          )}
        </p>
          ...
          <>
            ...
            <div className="modal-grid">
              <button
                type="button"
                className="modal-card"
                disabled={!getFluentProvider() || coreBusy || coreIsConnecting || coreIsConnected}
                onClick={() => void connectCore()}
                title={!getFluentProvider() ? 'Fluent is not installed' : 'Connect Core space'}
              >
                <div
                  style={{
                    width: 40,
