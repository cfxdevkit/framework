...
async function forward(request: Request, method: 'GET' | 'POST') {
  const path = new URL(request.url).pathname.split('/api/accounts/')[1] ?? '';
  if (!path) {
    return NextResponse.json(
      { error: 'Unknown accounts route', ok: false },
  ...
  const response =
    body === ''
      ? await requestRuntime(`/accounts/${path}${search}`, { method })
      : await requestRuntime(`/accounts/${path}${search}`, {
          body,
          headers,
          method,
        });
