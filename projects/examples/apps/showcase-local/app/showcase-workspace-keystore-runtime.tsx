...
import type { NetworkId, SpaceId } from './showcase-workspace-shared';
import { faucetFor, faucetLinksFor, syncWalletDrafts } from './showcase-workspace-shared';

type WorkspaceLog = (message: string, tone?: string) => void;
...
export function useShowcaseWorkspaceKeystoreRuntime({
  ...
  const devnodeBadge = useMemo(() => {
    ...
    if (devnode?.running) return <StatusBadge label="running" status="ok" />;
    if (devnodeBusy === 'refresh') return <StatusBadge label="loading" status="pending" />;
    return <StatusBadge label="stopped" status="info" />;
  }, [devnode, devnodeBusy, devnodeError]);

  const keystoreBadge = useMemo(() => {
    if (keystoreError) return <StatusBadge label="error" status="error" />;
    if (!keystoreStatus) return <StatusBadge label="loading" status="pending" />;
    if (!keystoreStatus.initialized) return <StatusBadge label="setup required" status="info" />;
    if (keystoreStatus.locked) return <StatusBadge label="locked" status="pending" />;
    if (activeWallet) return <StatusBadge label="signer ready" status="ok" />;
    return <StatusBadge label="unlocked" status="info" />;
